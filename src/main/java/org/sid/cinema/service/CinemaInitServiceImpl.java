...
public class CinemaInitServiceImpl implements ICinemaInitService {
	...
	public void initVilles() {
		Stream.of("Rabat", "Marrakech", "Casablanca", "Meknes", "Fes", "Tanger").forEach(nameVille -> {
	...
	public void initCinemas() {
		villeRepository.findAll().forEach(nameVille -> {
			Stream.of("Megarama", "CineAtlas", "IMAX", "Gueliz").forEach(nameCinema -> {
				Cinema cinema = new Cinema();
				cinema.setName(nameCinema);
				cinema.setNbrSalles(5 + (int) (Math.random() * 20));
	...
	public void initSalles() {
		cinemaRepository.findAll().forEach(nameCinema -> {
			for (int i = 0; i < nameCinema.getNbrSalles(); i++) {
				Salle salle = new Salle();
				salle.setName("Salle n° " + (i + 1));
				salle.setCinema(nameCinema);
				salle.setNbrPlaces(15 + (int) (Math.random() * 50));
	...
	public void initSeances() {
		...
		Stream.of("08:00", "10:30", "14:00", "17:30", "20:00", "22:30").forEach(s -> {
	...
	public void initCategories() {
		Stream.of("Drama", "Comédie", "Science-fiction", "Fantasie", "Action", "Horreur").forEach(cat -> {
	...
	public void initFilms() {
		double[] duree = new double[] { 1.5, 2, 2.15 };
		List<Categorie> categorie = categorieRepository.findAll();
		Stream.of("Kungfu Panda", "Baby Driver", "Mad Max", "Thor", "Into the Spiderverse", "Inglorious Basterds", "Moana", "Pulp Fiction").forEach(f -> {
			Film film = new Film();
			film.setTitre(f);
